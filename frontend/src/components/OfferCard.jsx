  ...
  return (
    <div
      className={`relative flex flex-col bg-white border-2 rounded-2xl overflow-hidden ${
        offer.recommended ? "border-[#2E7D32] shadow-md" : "border-[#E5E0D8]"
      } ${className}`}
      data-testid={`offer-card-${purpose}`}
    >
      {offer.recommended && (
        <span className="absolute top-0 right-0 bg-[#2E7D32] text-white text-[10px] font-bold uppercase px-2.5 py-1 rounded-bl-xl">
          Recommandé
        </span>
      )}
      <div className="p-4 sm:p-5 flex-1 flex flex-col" style={{ borderTop: `4px solid ${offer.color}` }}>
        <div
        ...
        <p className="font-heading font-bold text-xl sm:text-2xl mt-2" style={{ color: offer.color }}>
        ...
        <ul className={`mt-3 space-y-1.5 flex-1 ${compact ? "text-xs" : "text-sm"} text-[#4A5D50]`}>
          {offer.benefits.map((b) => (
            <li key={b} className="flex items-start gap-2">
              <Check size={compact ? 14 : 16} weight="bold" className="text-[#2E7D32] flex-shrink-0 mt-0.5" />
              <span>{b}</span>
            </li>
          ))}
        </ul>
        {disabled ? (
          ...
          <Link
            to={href}
            className={`mt-4 inline-flex items-center justify-center rounded-full font-bold text-center transition-colors ${
              compact ? "py-2.5 text-sm" : "py-3 text-sm sm:text-base"
            } ${offer.recommended ? "bg-[#2E7D32] hover:bg-[#1B5E20] text-white" : "text-white hover:opacity-90"}`}
            style={offer.recommended ? undefined : { backgroundColor: offer.color }}
